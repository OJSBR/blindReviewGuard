...
class LocaleFilesTest extends TestCase
{
    ...
    public const PLACEHOLDERS = [
        'log.findings' => ['{$total}', '{$filename}', '{$details}'],
        'log.cleaned' => ['{$removed}', '{$filename}', '{$details}'],
        'notification.findings' => ['{$filename}', '{$details}'],
        'notification.beforeReviewer' => ['{$filename}', '{$details}'],
    ];

    public const PREFIX = 'plugins.generic.blindReviewGuard.';
    ...
    public function testEveryKeyTheCodeUsesExists(): void
    {
        ...
        preg_match_all('/plugins\.generic\.blindReviewGuard\.[a-zA-Z.]+[a-zA-Z]/', $sources, $m);
    ...
    public function testUnreviewedLocalesAreFuzzyAndReviewedOnesAreNot(): void
    {
        foreach (self::LOCALES as $locale) {
            ...
            $expected = in_array($locale, self::REVIEWED, true) ? 0 : 18;
